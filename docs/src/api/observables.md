...
| [`MutualInformation`](@ref) | ``I(A\!:\!B) = S(A)+S(B)-S(A\cup B)`` for two contiguous, disjoint regions | Physical-site regions on every backend, cross-backend unambiguous under both boundary conditions (unlike `EntanglementEntropy`'s PBC `cut`); MPS size-guarded at ``d^{\lvert A\rvert+\lvert B\rvert} \le 256`` |
