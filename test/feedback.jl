...
@testset "Feedback system (Measure/OnOutcome/CallbackFeedback)" begin
    ...
    @testset "Random-unitary feedback (user's motivating case)" begin
        ...
        m = Measure(:Z; feedback=(s, sites, o) -> o == 1 && apply!(s, HaarRandom(1), SingleSite(sites[1])))
    ...
    @testset "Engine integration: Measure + feedback inside apply_with_prob!" begin
        ...
        st_a = mkstate(); simulate!(build(), st_a; n_steps=3, record_when=:final_only)
        st_b = mkstate(); simulate!(build_nofb(), st_b; n_steps=3, record_when=:final_only)
