...
json_sorted(d::Dict) = "{" * join(["\"$k\":$(JSON.json(v))" for (k,v) in sort(collect(d), by=first)], ",") * "}"
...
eA = case_a();  println("Case A entropy: first=$(eA[1]) last=$(eA[end])")
mz = case_c();  println("Case C Mz: first=$(mz[1]) last=$(mz[end]) (n=$(length(mz)))")
eD = case_d();  println("Case D entropy: first=$(eD[1]) last=$(eD[end])")
S, SO = case_aklt(); println("AKLT: S=$S |SO|=$(abs(SO))")
