...
function compare_case(name::String, produced::AbstractDict, golden::AbstractDict; atol=1e-14)
    ...
    for (key, gval) in golden
        key == "params" && continue
        haskey(produced, key) || (println("  [$name] MISSING key: $key"); pass = false; continue)
        pval = produced[key]
        ok = if gval isa AbstractVector
            length(pval) == length(gval) && all(abs.(Float64.(pval) .- Float64.(gval)) .<= atol)
