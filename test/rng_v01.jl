...
_fresh_registry(; spacetime=42, realization=2, born=1) =
...
@testset "RNG hygiene (v0.1)" begin
    ...
    @testset "with_guarded_stream blocks / allows / restores" begin
        ...
        draw(reg_a, :gates_spacetime); draw(reg_b, :gates_spacetime)
    ...
    @testset "expected_draws: unified-rule coin counts" begin
        ...
        srn = Circuit(L=L, bc=:periodic) do c
            apply_with_prob!(c; outcomes=[
                (probability=0.3, gate=Measurement(:Z), geometry=EachSite(2:L-1))
            ])
        end
        ...
        bad = Circuit(L=4, bc=:periodic, operations=NamedTuple[
