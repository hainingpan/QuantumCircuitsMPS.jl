...
function _eager_test_state(; L=8, bc=:periodic, gs=42, born=1, real=2, log_events=false)
...
@testset "Eager apply_with_prob! (v0.1 unified rule)" begin
    ...
    @testset "rng= kwarg hard-removed (migration error)" begin
        ...
        err = try
            apply_with_prob!(st; rng=:gates_realization, outcomes=[
        ...
        @test_throws ArgumentError apply_with_prob!(st; rng=:gates_spacetime, outcomes=[
            (probability=0.5, gate=PauliX(), geometry=SingleSite(1))])
    end
    ...
    @testset "unknown kwargs rejected" begin
        st = _eager_test_state(L=4)
        err = try
            apply_with_prob!(st; bogus=1, outcomes=[
                (probability=0.5, gate=PauliX(), geometry=SingleSite(1))])
            nothing
        catch e
            e
    ...
    @testset "Σp > 1 throws before any draw or mutation" begin
        ...
        @test rand(copy(get_rng(st.rng_registry, :gates_spacetime))) == rand(copy(rng_before))
    end
    ...
    @testset "equal-K mismatch throws before any draw or mutation" begin
        ...
        @test [born_probability(st, i, 0) for i in 1:4] == p_before
        @test rand(copy(get_rng(st.rng_registry, :gates_spacetime))) == rand(copy(rng_before))
        @test rand(copy(get_rng(st.rng_registry, :born_measurement))) == rand(copy(born_before))
    end
